...
describe('getTrainArrivals', () => {
  it('requests JSON output for the given platform stop', async () => {
    ...
    await getTrainArrivals('30960', 'test-key', { fetch, limit: 5, routes: ['Org', 'Red'] });
  ...
  it('rejects a non-numeric stop id', async () => {
    const { fetch, calls } = stubFetch(fixture('train-arrivals'));
    await expect(getTrainArrivals('Belmont', 'test-key', { fetch })).rejects.toThrow(TypeError);
    expect(calls).toHaveLength(0);
  });

  it('normalizes predictions and sorts them soonest-first', async () => {
    const { fetch } = stubFetch(fixture('train-arrivals'));
    const arrivals = await getTrainArrivals('30960', 'test-key', { fetch });
  ...
  it('turns an errCd body into a CtaApiError', async () => {
    ...
    await expect(getTrainArrivals('30960', 'test-key', { fetch })).rejects.toMatchObject({
      name: 'CtaApiError',
  ...
  it('returns an empty list when the station has no predictions', async () => {
    ...
    await expect(getTrainArrivals('30960', 'test-key', { fetch })).resolves.toEqual([]);
  });
  ...
  it('accepts a single eta object as well as an array', async () => {
    ...
    await expect(getTrainArrivals('30960', 'test-key', { fetch })).resolves.toHaveLength(1);
  });
  ...
  it('reports a non-JSON body rather than throwing a SyntaxError', async () => {
    ...
    await expect(getTrainArrivals('30960', 'test-key', { fetch })).rejects.toBeInstanceOf(CtaApiError);
  });
});
